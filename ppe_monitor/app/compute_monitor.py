...
class ComputeProfile:
    ...
    verifier_crop_gflops_per_infer: float = 20.0
    verifier_ollama_gflops_per_infer: float = 0.0
...
def estimate_compute_usage(
    *,
    ...
    verifier_crop_infer_per_sec: float,
    verifier_ollama_calls_per_sec: float,
) -> ComputeEstimate:
    if not enabled:
    ...
    verifier_crop_gflops_sec = max(0.0, verifier_crop_infer_per_sec) * max(0.0, profile.verifier_crop_gflops_per_infer)
    verifier_ollama_gflops_sec = max(0.0, verifier_ollama_calls_per_sec) * max(0.0, profile.verifier_ollama_gflops_per_infer)
    total_gflops_sec = (
        pose_gflops_sec
        + ppe_gflops_sec
        + verifier_aux_gflops_sec
        + verifier_crop_gflops_sec
        + verifier_ollama_gflops_sec
    )
    total_tflops_sec = total_gflops_sec / 1000.0
    total_tops_sec = total_tflops_sec
    total_flops_sec = total_gflops_sec * 1_000_000_000.0
    ...
    return ComputeEstimate(
        ...
        verifier_crop_infer_per_sec=round(float(verifier_crop_infer_per_sec), 2),
        verifier_ollama_calls_per_sec=round(float(verifier_ollama_calls_per_sec), 2),
        ...
        verifier_crop_estimated_gflops_per_sec=round(verifier_crop_gflops_sec, 2),
        verifier_ollama_estimated_gflops_per_sec=round(verifier_ollama_gflops_sec, 2),
        estimated_gflops_per_sec=round(total_gflops_sec, 2),
        estimated_tflops_per_sec=round(total_tflops_sec, 4),
        estimated_tops_per_sec=round(total_tops_sec, 4),
        estimated_flops_per_sec=round(total_flops_sec, 0),
